...
std::string GetUniqueHost(Env* env, absl::string_view run_dir,
  ...
  for (int i = 1; i <= kMaxAttempts; ++i) {
    std::string new_hostname = absl::StrCat(hostname, "_", i);
    std::string new_file_name = absl::StrCat(new_hostname, ".", kXPlanePb);
    std::string new_out_path = ProfilerJoinPath(run_dir, new_file_name);
    if (!env->FileExists(new_out_path).ok()) {
      return new_hostname;
    }
  }
