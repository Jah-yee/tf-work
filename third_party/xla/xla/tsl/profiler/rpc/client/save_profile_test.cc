...
TEST(SaveProfileTest, SaveXSpaceCollision) {
  ...
  ASSERT_TRUE(status.ok());
  EXPECT_TRUE(Env::Default()
                  ->FileExists(ProfilerJoinPath(log_dir, "my_host_1.xplane.pb"))
                  .ok());

  // 3. Save third profile with same host
  status = SaveXSpace(repository_root, run, host, xspace);
  ASSERT_TRUE(status.ok());
  EXPECT_TRUE(Env::Default()
                  ->FileExists(ProfilerJoinPath(log_dir, "my_host_2.xplane.pb"))
                  .ok());
}
...
TEST(SaveProfileTest, SaveProfileCollision) {
  ...
  EXPECT_TRUE(
      Env::Default()
          ->FileExists(ProfilerJoinPath(log_dir, "my_host_1.trace.json"))
          .ok());
}

TEST(SaveProfileTest, SaveXSpaceMaxCollisionsFallback) {
  std::string repository_root = TmpDir();
  std::string run =
      absl::StrCat("test_run_fallback_", absl::ToUnixMicros(absl::Now()));
  std::string host = "my_host";
  tensorflow::profiler::XSpace xspace;
  ...
  for (int i = 1; i <= 100; ++i) {
    ASSERT_TRUE(WriteStringToFile(
                    Env::Default(),
                    ProfilerJoinPath(log_dir,
                                     absl::StrCat("my_host_", i, ".xplane.pb")),
                    "")
                    .ok());
  }
  ...
  for (const auto& file : files) {
    if (absl::StartsWith(file, "my_host_")) {
      absl::string_view suffix = absl::StripPrefix(file, "my_host_");
      suffix = absl::StripSuffix(suffix, ".xplane.pb");
      int val;
      if (!absl::SimpleAtoi(suffix, &val) || val > 100) {
        found_fallback = true;
        break;
      }
    }
